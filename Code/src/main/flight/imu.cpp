...
static void imuCalculateEstimatedAttitude ( void ) {
  ...
  //   }
  //   if ( imuRuntimeConfig->gyro_lpf_factor > 0 ) {
  //     gyroLPF [ axis ]            = gyroLPF [ axis ] * ( 1.0f - ( 1.0f / imuRuntimeConfig->gyro_lpf_factor ) ) + gyroADC [ axis ] * ( 1.0f / imuRuntimeConfig->gyro_lpf_factor );
  //     deltaGyroAngle.raw [ axis ] = gyroLPF [ axis ] * scale;
  //   } else {
  ...
  //!
  //! NEW : Initialization With Acc & Gyro LPF Filter
  //!
  for ( axis = 0; axis < 3; axis++ ) {
    if ( imuRuntimeConfig->acc_lpf_factor > 0 ) {
      accLPF [ axis ]    = accLPF [ axis ] * ( 1.0f - ( 1.0f / imuRuntimeConfig->acc_lpf_factor ) ) + accADC [ axis ] * ( 1.0f / imuRuntimeConfig->acc_lpf_factor );
  ...
  // }
  //!
  //! NEW : Smoothly blend accelerometer data into the complementary filter
  //!
  float accWeight = 1.0f;
  if ( accMag < 72 || accMag > 133 ) {
    accWeight = 0.1f;    // light trust instead of total rejection
  }
  for ( axis = 0; axis < 3; axis++ ) {
    EstG.A [ axis ] = ( EstG.A [ axis ] * imuRuntimeConfig->gyro_cmpf_factor + accSmooth [ axis ] * accWeight ) * invGyroComplimentaryFilterFactor;
  }
...
void normalize ( void ) {
  ...
  if ( ! renorm ( t0, &dcm_matrix [ 0 ][ 0 ] ) || ! renorm ( t1, &dcm_matrix [ 1 ][ 0 ] ) || ! renorm ( t2, &dcm_matrix [ 2 ][ 0 ] ) ) {
...
void check_matrix ( void ) {
  ...
  // error_course value.
  if ( ! ( dcm_matrix [ 2 ][ 0 ] < 1.0f && dcm_matrix [ 2 ][ 0 ] > -1.0f ) ) {
    // We have an invalid matrix. Force a normalisation.
    normalize ( );

    if ( isNan || fabsf ( dcm_matrix [ 2 ][ 0 ] ) > 10 ) {
