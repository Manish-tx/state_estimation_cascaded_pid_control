...
static void pidMultiWii(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig);
...
static void pidLuxFloat(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
...
#ifdef GPS
            errorAngle = (constrain(rcCommand[axis] + GPS_angle[axis], -((int) max_angle_inclination), +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis]) / 10.0f; // 16 bits is ok here
#else
                    errorAngle = (constrain(rcCommand[axis], -((int) max_angle_inclination),
                                    +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis]) / 10.0f; // 16 bits is ok here
#endif
...
#ifdef GPS
            errorAngle = constrain(2 * rcCommand[axis] + GPS_angle[axis], -((int) max_angle_inclination), +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis];
#else
            errorAngle = constrain(2 * rcCommand[axis], -((int) max_angle_inclination),
                    +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis];
#endif
...
static void pidMultiWii23(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    UNUSED(rxConfig);
...
static void pidMultiWiiHybrid(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    UNUSED(rxConfig);
...
static void pidHarakiri(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    UNUSED(rxConfig);
...
#ifdef GPS
            error = constrain(2.0f * rcCommandAxis + GPS_angle[axis], -((int) max_angle_inclination), +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis];
#else
            error = constrain(2.0f * rcCommandAxis, -((int) max_angle_inclination), +max_angle_inclination) - inclination_generalised.raw[axis] + angleTrim->raw[axis];
#endif
...
float get_filt_alpha(uint16_t dt)
{
...
static void pidRewrite(pidProfile_t *pidProfile, controlRateConfig_t *controlRateConfig, uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (axis == FD_YAW) {
            ...
            desiredRate[axis] = (((int32_t)(rate + 20) * rcCommand[YAW]) >> 5);
        } else {
            ...
            if (!FLIGHT_MODE(ANGLE_MODE)) { //control is GYRO based (ACRO and HORIZON - direct sticks control is applied to rate PID
                desiredRate[axis] = ((int32_t)(rate + 20) * rcCommand[axis]) >> 4;
                if (FLIGHT_MODE(HORIZON_MODE)) {
...
//        else
        ...
        /* DTerm calculation adopted from Ardupilot */

        //-----calculate D-term
        if (dt > 0.0f) {
            derivative = (gyroADC[axis] - lastGyroReadings[axis]);
            derivatives[axis] = derivatives[axis] + (6 * (derivative - derivatives[axis]) / 10);

        }

        lastGyroReadings[axis] = gyroADC[axis];
