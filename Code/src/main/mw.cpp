...
void loop ( void ) {
    ...
    static uint32_t lastLaserTime1 = 0;
    uint32_t now1 = micros();

    // Update every 30 ms (33 Hz) – safe for VL53L1X
    if (now1 - lastLaserTime1 >= 10000) {
        lastLaserTime1 = now1;

        // if (XVision.startRanging()) {
        //     int16_t laser_mm = XVision.getLaserRange();
        //     Monitor_Print(" Laser(mm):", laser_mm); 
        // }
        // Monitor_Print(" OPticalFlow:",flowRate[0]);
        // Monitor_Print(" BARO:",baroCalculateAltitude());
        // Monitor_Print(" PITCH:",inclination.values.pitchDeciDegrees);
        // Monitor_Print(" ROLL:",inclination.values.rollDeciDegrees);
        // Monitor_Print(" YAW:",heading,4);
        ...
        //    now1,accADC[0], accADC[1], accADC[2], gyroADC[0], gyroADC[1], gyroADC[2],
        //   inclination.values.rollDeciDegrees, inclination.values.pitchDeciDegrees, heading);
        // Monitor_Print(str);
    }
